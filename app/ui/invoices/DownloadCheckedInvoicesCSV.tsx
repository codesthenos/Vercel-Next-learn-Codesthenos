...
import { downloadCheckedCSV } from "@/app/lib/actions"
import { useState } from "react"

export default function DownloadCheckedButton ({ query }: { query: string }) {
  ...
  const handleClick = async () => {
    ...
    try {
      ...
      const response = await downloadCheckedCSV(query)
      if (response.error) throw new Error(response.error)
      // create the blob (binary object with content [fileContent] and options { type: ... })
      if (response.fileContent) {
        const blob = new Blob([response.fileContent], { type: 'text/csv' })
        ...
        link.download = `invoice-${query}.csv`
